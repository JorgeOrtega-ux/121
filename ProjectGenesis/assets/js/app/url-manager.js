...
async function loadPage(page, action, fetchParams = null, isPartialLoad = false, runPageInitLogic = true) {
    ...
    try {
        ...
        // --- ▼▼▼ INICIO DE MODIFICACIÓN (LÓGICA DE SOMBRA PARA CHAT) ▼▼▼ ---
        if (page === 'messages') {
            const chatHeader = contentContainer.querySelector('.chat-content-header');
            const chatScroller = contentContainer.querySelector('.chat-message-list.overflow-y');

            if (chatHeader && chatScroller) {
...
export function initRouter() {
    ...
    document.body.addEventListener('click', e => {
        ...
        // --- ▼▼▼ INICIO DE LA MODIFICACIÓN (AÑADIR SELECTOR) ▼▼▼ ---
        const link = e.target.closest(
            ...
            'button[data-href*="/messages"][data-nav-js="true"], ' + // <-- LÍNEA AÑADIDA
