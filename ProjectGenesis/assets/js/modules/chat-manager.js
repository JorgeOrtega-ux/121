...
function renderConversationList(conversations) {
    ...
    conversations.forEach(convo => {
        ...
        // Usar la propiedad 'is_online' que viene de la API
...
async function loadConversations(filter = 'all') {
    ...
    try {
        ...
        if (result.success) {
            ...
            // --- ▼▼▼ INICIO DE MODIFICACIÓN (JS) ▼▼▼ ---
            // El bloque try...catch que llamaba a callFriendApi ha sido eliminado.
            
            // conversations.forEach(convo => {
            //     convo.is_online = !!onlineUserIds[convo.friend_id];
            // });
            // La propiedad convo.is_online ahora viene directamente de `result.conversations`
            // --- ▲▲▲ FIN DE MODIFICACIÓN (JS) ▲▲▲ ---
...
/**
 * Ajusta la altura del textarea expandido basado en su contenido.
 * @param {HTMLTextAreaElement} el El elemento textarea.
 */
function autoGrowTextarea(el) {
    if (!el || el.id !== 'chat-message-input-expanded') {
        return;
    }
...
/**
 * Habilita o deshabilita el input de chat.
 * @param {boolean} allow - True para habilitar, false para deshabilitar.
 * @param {string|null} reason - Clave i18n para el placeholder si se deshabilita.
 */
function enableChatInput(allow, reason = null) {
    const expandedInput = document.getElementById('chat-message-input-expanded');
    // --- CORRECCIÓN 1: ID incorrecto ---
    const attachBtn = document.getElementById('chat-attach-menu-button'); 
    const sendBtn = document.getElementById('chat-send-button');
    
    if (!expandedInput || !attachBtn || !sendBtn) {
        console.error("Error: Faltan elementos del DOM en enableChatInput", expandedInput, attachBtn, sendBtn);
        return;
    }
    ...
    if (allow) {
        ...
        selectedAttachments = [];
        document.getElementById('chat-attachment-preview-container')?.remove();
...
/**
 * Valida si el botón de enviar debe estar activo.
 */
function validateSendButton() {
    const expandedInput = document.getElementById('chat-message-input-expanded');
    const sendBtn = document.getElementById('chat-send-button');
    if (!sendBtn || !expandedInput) return;

    // Comprobar si el input está deshabilitado
    if (expandedInput.disabled) {
        sendBtn.disabled = true;
        return;
    }
    
    const hasText = expandedInput.value.trim().length > 0;
...
/**
 * Crea la vista previa de un adjunto.
 * @param {File} file - El archivo a previsualizar.
 */
function createAttachmentPreview(file) {
    let container = document.getElementById('chat-attachment-preview-container');
    const pillWrapper = document.querySelector('.chat-input-pill-wrapper'); 

    if (!container) {
        container = document.createElement('div');
        container.id = 'chat-attachment-preview-container';
        container.className = 'chat-attachment-preview-container';
        
        if (pillWrapper) {
            pillWrapper.prepend(container); 
        } else {
            console.error("No se encontró '.chat-input-pill-wrapper' para adjuntar la vista previa.");
            return; 
        }
    }
...
function showReplyPreview(messageId, username, text) {
    let container = document.getElementById('chat-reply-preview-container');
    
    const expandedInput = document.getElementById('chat-message-input-expanded');
    if (expandedInput && expandedInput.disabled) {
        return;
    }
    
    if (!container) {
        container = document.createElement('div');
        container.id = 'chat-reply-preview-container';
        container.className = 'chat-reply-preview-container';
        
        const form = document.getElementById('chat-message-input-form');
        if (form) {
            form.prepend(container); 
        } else {
            console.error("No se encontró '#chat-message-input-form' para adjuntar la vista previa de respuesta.");
            return;
        }
    }
    ...
    document.getElementById('chat-message-input-expanded')?.focus();
}
...
/**
 * Envía el mensaje actual (texto y/o archivos).
 */
async function sendMessage() {
    ...
    if (!currentChatTargetId || !sendBtn || sendBtn.disabled || !expandedInput) {
        return;
    }
    ...
    sendBtn.disabled = true;
    expandedInput.disabled = true;
    // --- CORRECCIÓN 2: ID incorrecto ---
    document.getElementById('chat-attach-menu-button').disabled = true;
    ...
    try {
        ...
        if (result.success && result.message_sent) {
            ...
            // await loadConversations(currentChatFilter); // <-- LÍNEA COMENTADA
            
            // En lugar de recargar toda la lista, actualizamos el caché y volvemos a renderizar
            const sentMessage = result.message_sent;
            const convoId = parseInt(sentMessage.receiver_id, 10); // El ID del amigo al que le enviamos

            const friendItem = friendCache.find(f => f.friend_id == convoId);
            
            if (friendItem) {
                // Actualizar el caché
                friendItem.last_message = sentMessage.attachment_urls ? (sentMessage.message_text || getTranslation('chat.snippet.image')) : sentMessage.message_text;
                if(sentMessage.status === 'deleted') friendItem.last_message = getTranslation('chat.snippet.deleted');
                friendItem.last_message_time = sentMessage.created_at;
                friendItem.is_archived = false; // El envío desarchiva
                friendItem.pinned_at = friendItem.pinned_at || null; // Mantener el pin
                
                // Re-ordenar: Mover este item al principio (después de los anclados)
                friendCache = friendCache.filter(f => f.friend_id != convoId); // Quitarlo
                
                // Encontrar el índice del último anclado
                let lastPinnedIndex = -1;
                for (let i = 0; i < friendCache.length; i++) {
                    if (friendCache[i].pinned_at) {
                        lastPinnedIndex = i;
                    } else {
                        break; // Parar en el primer no-anclado
                    }
                }
                
                friendCache.splice(lastPinnedIndex + 1, 0, friendItem); // Insertarlo justo después del último anclado

                // Volver a renderizar la lista desde el caché
                const searchInput = document.getElementById('chat-friend-search');
                const currentQuery = searchInput ? searchInput.value : '';
                filterConversationList(currentQuery);
                
            } else {
                // Si por alguna razón no estaba en el caché (chat nuevo), recargamos
            ...
            autoGrowTextarea(expandedInput); // Ajustar el expandido (ahora vacío)

            enableChatInput(true);
            expandedInput.focus(); // Enfocar el expandido
...
export function handleChatMessageReceived(message) {
    ...
    const friendItem = friendCache.find(f => f.friend_id == senderId);
    
    // 1. Decidir si el chat está abierto o no
    if (senderId === currentChatTargetId) {
        // Chat ESTÁ abierto
        const bubbleHtml = createMessageBubbleHtml(message, false);
        document.getElementById('chat-message-list').insertAdjacentHTML('beforeend', bubbleHtml);
        scrollToBottom();
        
        // Como está abierto, marcamos como leído en la API (esto es opcional, pero bueno)
        // O mejor, lo dejamos para cuando el usuario abra el chat (loadChatHistory lo hace)
        // PERO SÍ debemos actualizar el caché para que el snippet se actualice en la lista
        if (friendItem) {
             friendItem.last_message = message.attachment_urls ? (message.message_text || getTranslation('chat.snippet.image')) : message.message_text;
             if(message.status === 'deleted') friendItem.last_message = getTranslation('chat.snippet.deleted');
             friendItem.last_message_time = message.created_at;
             friendItem.is_archived = false; 
             friendItem.pinned_at = friendItem.pinned_at || null;
             
             // Re-ordenar
             friendCache = friendCache.filter(f => f.friend_id != senderId);
             let lastPinnedIndex = -1;
             for (let i = 0; i < friendCache.length; i++) {
                if (friendCache[i].pinned_at) lastPinnedIndex = i;
                else break;
             }
             friendCache.splice(lastPinnedIndex + 1, 0, friendItem);
            ...
            // Raro, pero si no está en el caché, recargamos
            loadConversations(currentChatFilter);
        }
        
    } else {
        // Chat NO está abierto
        ...
        if (friendItem) {
            ...
            friendItem.is_archived = false;
            friendItem.pinned_at = friendItem.pinned_at || null;
            friendItem.unread_count = (parseInt(friendItem.unread_count, 10) || 0) + 1;
            ...
            // Es un chat 100% nuevo, recargamos de la DB
            loadConversations(currentChatFilter);
        }
    }
    // --- FIN DE MODIFICACIÓN ---
...
export function initChatManager() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        const attachPhotosBtn = e.target.closest('[data-action="chat-attach-photos"]');
        if (attachPhotosBtn) {
            e.preventDefault();
            e.stopPropagation(); 
        ...
        // --- CORRECCIÓN 3: ID incorrecto ---
        const attachBtn = e.target.closest('#chat-attach-menu-button'); 
        if (attachBtn) {
            e.preventDefault();
            
            // --- ESTA ES LA LÓGICA QUE FALTABA ---
            // El data-action="toggleModuleChatAttach" está en el HTML,
            // pero el main-controller no lo maneja. Lo manejamos aquí.
            e.stopPropagation();
            const module = document.querySelector(`[data-module="moduleChatAttach"]`);
            if (module) {
    ...
    document.body.addEventListener('input', (e) => {
        // Escuchar solo en el textarea expandido
        const chatInput = e.target.closest('#chat-message-input-expanded'); 
        if (chatInput) {
            validateSendButton();
    ...
    document.addEventListener('user-presence-changed', (e) => {
        ...
        // --- ▼▼▼ INICIO DE MODIFICACIÓN ▼▼▼ ---
        // El selector original '.friend-item' es para la lista de amigos (home).
        // El selector correcto para la lista de chat (messages) es '.chat-conversation-item'.
        const chatConversationItem = document.querySelector(`.chat-conversation-item[data-target-id="${userId}"]`);
        if (chatConversationItem) {
            // El dot en la lista de chat tiene la clase '.chat-item-status'
            const dot = chatConversationItem.querySelector('.chat-item-status');
            if (dot) {
