...
export function updateProfileActions(userId, newStatus) {
    ...
    // 1. Encontrar y eliminar SOLO los botones de amistad antiguos
    const oldFriendButtons = actionsContainer.querySelectorAll('[data-action^="friend-"]');
    oldFriendButtons.forEach(btn => btn.remove());

    // 2. Construir el HTML SOLO para los NUEVOS botones de amistad
    let newHtml = '';
    ...
    // 3. Insertar el nuevo HTML al *principio* del contenedor
    // (El botón de mensaje queda al final, como en el renderizado PHP)
    actionsContainer.insertAdjacentHTML('afterbegin', newHtml);

    // 4. Actualizar el estilo del botón de mensaje (que no fue eliminado)
    // --- ESTE ES EL SELECTOR CORREGIDO ---
    const messageButton = actionsContainer.querySelector('button[data-nav-js="true"][data-href*="/messages/"]');
    if (messageButton) {
        if (newStatus === 'friends') {
            // Si somos amigos, hacerlo primario
            messageButton.classList.add('component-button--primary');
        } else {
            // Si no somos amigos (o pendiente), quitarle lo primario
            messageButton.classList.remove('component-button--primary');
        }
    }
}
