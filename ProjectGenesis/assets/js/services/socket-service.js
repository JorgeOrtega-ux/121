...
function connectWebSocket() {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            // Añadimos el auth_token (de main-layout.php) al mensaje de autenticación.
            // El session_id sigue siendo el csrfToken, pero el auth_token es el token de sesión real.
            sendSocketMessage({
                type: "auth",
                user_id: window.userId || 0,
                session_id: window.csrfToken || "", // Este es el CSRF token
                auth_token: window.userAuthToken || "" // <-- NUEVA LÍNEA
            });
            // --- ▲▲▲ FIN DE MODIFICACIÓN ▲▲▲ ---
        };
