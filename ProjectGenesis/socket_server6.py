...
# --- ▼▼▼ INICIO DE MODIFICACIÓN (AUTENTICACIÓN DE WEBSOCKET) ▼▼▼ ---
# URL del validador interno de PHP
PHP_AUTH_VALIDATOR_URL = "http://127.0.0.1/ProjectGenesis/api/auth_handler.php"

async def validate_php_token(user_id, auth_token, session_id):
    """Llama a la API de PHP para validar el token de un usuario."""
    # Este payload debe coincidir con el que espera auth_handler.php
    payload = {
        "action": "internal-ws-validate-token",
        "user_id": user_id,
        "auth_token": auth_token,
        "session_id": session_id # Pasamos el session_id (csrf) para un log completo si es necesario
    }
    
    try:
        # Usamos aiohttp para hacer una solicitud HTTP asíncrona interna
        async with aiohttp.ClientSession() as session:
            # Hacemos un POST con un timeout corto (el servidor PHP está en la misma máquina)
            async with session.post(PHP_AUTH_VALIDATOR_URL, json=payload, timeout=2.0) as response:
                if response.status == 200:
                    result = await response.json()
                    # Comprobamos que PHP nos dé 'ok' y que el user_id coincida
                    if result.get("status") == "ok" and result.get("user_id") == user_id:
                        logging.info(f"[AUTH] Validación exitosa para user_id={user_id}")
                        return True
                    else:
                        logging.warning(f"[AUTH] Validación fallida para user_id={user_id}. Respuesta PHP: {result.get('message')}")
                        return False
                else:
                    logging.error(f"[AUTH] Error al contactar validador PHP (código {response.status}) para user_id={user_id}")
                    return False
    except Exception as e:
        # Esto puede fallar si PHP no está corriendo o la URL es incorrecta
        logging.error(f"[AUTH] Excepción al contactar validador PHP: {e}")
        return False
# --- ▲▲▲ FIN DE MODIFICACIÓN ▲▲▲ ---
...
async def ws_handler(websocket):
    ...
    try:
        ...
        if (data.get("type") == "auth" and 
            ...
            data.get("auth_token")): # <-- NUEVA LÍNEA
            
            user_id_from_client = int(data["user_id"])
            session_id_from_client = data["session_id"]
            auth_token_from_client = data["auth_token"] # <-- NUEVA LÍNEA

            # 2. Validar el token contra el backend de PHP
            is_valid = await validate_php_token(user_id_from_client, auth_token_from_client, session_id_from_client)
            
            if is_valid:
                # 3. El token es válido, proceder con el registro
                user_id = user_id_from_client # Asignar a la variable de alcance superior
                session_id = session_id_from_client # Asignar a la variable de alcance superior
                await register_client(websocket, user_id, session_id)
            else:
                # 4. El token es inválido, rechazar y cerrar la conexión
                raise Exception(f"Autenticación fallida: Token inválido o no coincide con user_id={user_id_from_client}")
        # --- ▲▲▲ FIN DE MODIFICACIÓN ▲▲▲ ---
        else:
