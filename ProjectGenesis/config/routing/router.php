        ...
        if ($targetUserUuid) { 
            try {
                ...
                if ($targetUser) {
                    ...
                    // --- ▼▼▼ INICIO DE MODIFICACIÓN (AÑADIR CONSULTA A SOCKET) ▼▼▼ ---
                    $is_actually_online = false; // Default
                    try {
                        // Usar un timeout corto (0.5s) para no bloquear la carga de la página
                        ...
                        if ($jsonResponse !== false) {
                            ...
                            if (isset($data['status']) && $data['status'] === 'ok' && isset($data['online_users'])) {
                                // Comprobar si el ID del *objetivo* está en la lista de IDs online
                                $is_actually_online = in_array($targetUserId, $data['online_users']);
                            }
                        }
                    } catch (Exception $e) {
                        // Si el socket server no responde, $is_actually_online seguirá 'false'
                        logDatabaseError($e, 'router - messages (ws_get_online_fail)');
                    }
                    // --- ▲▲▲ FIN DE MODIFICACIÓN ▲▲▲ ---
