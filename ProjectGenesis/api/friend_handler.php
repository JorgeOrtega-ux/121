    ...
    try {
        if ($action === 'get-friends-list') {
            ...
            try {
                ...
                // 1. Notificación para actualizar el estado de los botones (si el usuario está viendo el perfil)
                $payload_state = [
                    'type'          => 'friend_status_update',
                    'actor_user_id' => $currentUserId,
                    'new_status'    => 'pending_received'
                ];
                notifyUser($targetUserId, $payload_state);

                // 2. Notificación PING para activar la campana de notificación
                $payload_ping = [
                    'type' => 'new_notification_ping'
                ];
                notifyUser($targetUserId, $payload_ping);
                // --- FIN DE CORRECCIÓN ---
