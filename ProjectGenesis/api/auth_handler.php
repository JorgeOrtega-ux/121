...
// Nota: 'Content-Type: application/json' se define más abajo para la mayoría de las rutas,
// pero el validador de WS lo definirá él mismo.

$response = ['success' => false, 'message' => 'js.api.invalidAction'];

// --- ▼▼▼ INICIO DE CORRECCIÓN (AUTENTICACIÓN DE WEBSOCKET) ▼▼▼ ---

// 1. Leer el payload JSON crudo PRIMERO.
$json_data = file_get_contents('php://input');
$post_data = json_decode($json_data, true);

// 2. Determinar la acción. Dar prioridad al JSON (para WS), luego a $_POST (para web).
$action = $post_data['action'] ?? $_POST['action'] ?? '';

// Si la acción es la validación interna del WebSocket...
if ($action === 'internal-ws-validate-token') {
    // Establecer el header aquí
    header('Content-Type: application/json');

    // 1. Comprobar IP. Solo permitir localhost (donde corre el server.py)
    $allowed_ips = ['127.0.0.1', '::1'];
    if (!in_array($_SERVER['REMOTE_ADDR'], $allowed_ips)) {
        http_response_code(403);
        logDatabaseError(new Exception("Llamada no autorizada a ws-validate desde " . $_SERVER['REMOTE_ADDR']), 'auth_handler');
        echo json_encode(['status' => 'error', 'message' => 'Forbidden']);
        exit;
    }

    // 2. Obtener datos (YA LOS LEÍMOS ARRIBA)
    $user_id = (int)($post_data['user_id'] ?? 0);
    $auth_token = $post_data['auth_token'] ?? '';
    $session_id = $post_data['session_id'] ?? ''; // El CSRF token (para logs)

    $validation_response = ['status' => 'error', 'message' => 'Invalid credentials'];

    if (empty($user_id) || empty($auth_token) || empty($session_id)) {
        $validation_response['message'] = 'Missing fields';
        echo json_encode($validation_response);
        exit;
    }
    
    try {
        // 3. Validar el auth_token contra la BD
        $stmt = $pdo->prepare(
            "SELECT id FROM users 
             WHERE id = ? AND auth_token = ? AND account_status = 'active'"
        );
        $stmt->execute([$user_id, $auth_token]);
        $user = $stmt->fetch();

        if ($user) {
            // 4. Éxito: El token y el ID coinciden.
            $validation_response['status'] = 'ok';
            $validation_response['message'] = 'Token valid';
            $validation_response['user_id'] = (int)$user['id'];
        } else {
            // 5. Fracaso: El token o el ID son incorrectos.
            logDatabaseError(new Exception("Validacion WS fallida para user_id $user_id"), 'auth_handler_ws_validate');
            $validation_response['message'] = 'Token or user_id mismatch';
        }

    } catch (PDOException $e) {
        logDatabaseError($e, 'auth_handler - internal-ws-validate-token');
        $validation_response['message'] = 'Database error';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Esta validación ahora se aplica a todas las acciones EXCEPTO
    // 'internal-ws-validate-token' (que se manejó arriba)
    ...
    // $action ya fue definido al inicio del script
    if (isset($action)) {
    
        // $action = $_POST['action']; // <--- Esta línea ya se ejecutó al inicio del script
